...
public class SlidingTabLayout
	...
	private void obtainAttributes(Context context, AttributeSet attrs) {
		...
		mUnderlineHeight = ta.getDimension(
				R.styleable.SlidingTabLayout_tl_underline_height,
				dp2px(0)
										  );
		...
		mDividerWidth = ta.getDimension(
				R.styleable.SlidingTabLayout_tl_divider_width,
				dp2px(0)
									   );
		mDividerPadding = ta.getDimension(
				R.styleable.SlidingTabLayout_tl_divider_padding,
				dp2px(12)
										 );
		...
		mTextAllCaps = ta.getBoolean(
				R.styleable.SlidingTabLayout_tl_textAllCaps,
				false
									);
		...
		mTabWidth = ta.getDimension(
				R.styleable.SlidingTabLayout_tl_tab_width,
				dp2px(-1)
								   );
	...
	public void setViewPager(ViewPager vp, String[] titles) {
		...
		if (titles.length != vp.getAdapter()
							   .getCount()) {
	...
	private void scrollToCurrentTab() {
		...
		int newScrollX = mTabsContainer.getChildAt(mCurrentTab)
									   .getLeft() + offset;
	...
	private void calcIndicatorRect() {
		...
		if (this.mCurrentTab < mTabCount - 1) {
			...
			if (mIndicatorStyle == STYLE_NORMAL && mIndicatorWidthEqualTitle) {
				...
				mTextPaint.setTextSize(mTextsize);
				float nextTextWidth = mTextPaint.measureText(next_tab_title.getText()
																		   .toString());
