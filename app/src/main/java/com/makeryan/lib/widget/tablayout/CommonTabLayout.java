...
import com.makeryan.lib.widget.tablayout.utils.FragmentChangeManager;
import com.makeryan.lib.widget.tablayout.utils.UnreadMsgUtils;
import com.makeryan.lib.widget.tablayout.widget.MsgView;
...
public class CommonTabLayout
	...
	private ArrayList<ITabBody> mTabEntitys = new ArrayList<>();
	...
	private void obtainAttributes(Context context, AttributeSet attrs) {
		...
		mIndicatorCornerRadius = ta.getDimension(
				...
				dp2px(mIndicatorStyle == STYLE_BLOCK ?
							  -1 :
							  0)
												);
		...
		mIndicatorAnimDuration = ta.getInt(
				R.styleable.CommonTabLayout_tl_indicator_anim_duration,
				-1
										  );
		...
		mUnderlineHeight = ta.getDimension(
				R.styleable.CommonTabLayout_tl_underline_height,
				dp2px(0)
										  );
		...
		mDividerPadding = ta.getDimension(
				R.styleable.CommonTabLayout_tl_divider_padding,
				dp2px(12)
										 );

		mTextsize = ta.getDimension(
				R.styleable.CommonTabLayout_tl_textsize,
				sp2px(13f)
								   );
		...
		mTextAllCaps = ta.getBoolean(
				R.styleable.CommonTabLayout_tl_textAllCaps,
				false
									);

		mIconVisible = ta.getBoolean(
				R.styleable.CommonTabLayout_tl_iconVisible,
				true
									);
		mIconGravity = ta.getInt(
				R.styleable.CommonTabLayout_tl_iconGravity,
				Gravity.TOP
								);
		mIconWidth = ta.getDimension(
				R.styleable.CommonTabLayout_tl_iconWidth,
				dp2px(0)
									);
		mIconHeight = ta.getDimension(
				R.styleable.CommonTabLayout_tl_iconHeight,
				dp2px(0)
									 );
		mIconMargin = ta.getDimension(
				R.styleable.CommonTabLayout_tl_iconMargin,
				dp2px(2.5f)
									 );
		...
		mTabWidth = ta.getDimension(
				R.styleable.CommonTabLayout_tl_tab_width,
				dp2px(-1)
								   );
	...
	public void setTabData(ArrayList<ITabBody> tabEntitys) {
	 ...
	 * 关联数据支持同时切换fragments
	 */
	public void setTabData(ArrayList<ITabBody> tabEntitys, FragmentActivity fa, int containerViewId, ArrayList<Fragment> fragments) {

		mFragmentChangeManager = new FragmentChangeManager(
	...
	private void addTab(final int position, View tabView) {
		...
		tv_tab_title.setText(mTabEntitys.get(position)
										.getTabTitle());
		ImageView iv_tab_icon = (ImageView) tabView.findViewById(R.id.iv_tab_icon);
		iv_tab_icon.setImageResource(mTabEntitys.get(position)
												.getTabUnselectedIcon());
		tv_tab_title.setOnClickListener(v -> tabView.performClick());
		iv_tab_icon.setOnClickListener(v -> tabView.performClick());
		...
		if (mTabWidth > 0) {
			lp_tab = new LinearLayout.LayoutParams(
					(int) mTabWidth,
					LayoutParams.MATCH_PARENT
			);
		}
	...
	private void updateTabStyles() {
		...
		for (int i = 0; i < mTabCount; i++) {
			...
			TextView tv_tab_title = (TextView) tabView.findViewById(R.id.tv_tab_title);
			tv_tab_title.setTextColor(i == mCurrentTab ?
											  mTextSelectColor :
											  mTextUnselectColor);
			...
			if (mTextAllCaps) {
				tv_tab_title.setText(tv_tab_title.getText()
												 .toString()
												 .toUpperCase());
			}

			if (mTextBold == TEXT_BOLD_BOTH) {
				tv_tab_title.getPaint()
			...
			if (mIconVisible) {
				...
				ITabBody tabEntity = mTabEntitys.get(i);
				iv_tab_icon.setImageResource(i == mCurrentTab ?
													 tabEntity.getTabSelectedIcon() :
	...
	private void updateTabSelection(int position) {
		...
		for (int i = 0; i < mTabCount; ++i) {
			...
			tab_title.setTextColor(isSelect ?
										   mTextSelectColor :
										   mTextUnselectColor);
			ImageView iv_tab_icon = (ImageView) tabView.findViewById(R.id.iv_tab_icon);
			ITabBody  tabEntity   = mTabEntitys.get(i);
			iv_tab_icon.setImageResource(isSelect ?
												 tabEntity.getTabSelectedIcon() :
												 tabEntity.getTabUnselectedIcon());
			if (mTextBold == TEXT_BOLD_WHEN_SELECT) {
				tab_title.getPaint()
						 .setFakeBoldText(isSelect);
			}
		}
	}
	...
	private void calcOffset() {
		...
		if (mLastP.left == mCurrentP.left && mLastP.right == mCurrentP.right) {
			...
			mValueAnimator.setObjectValues(
					mLastP,
					mCurrentP
										  );
			...
			if (mIndicatorAnimDuration < 0) {
				mIndicatorAnimDuration = mIndicatorBounceEnable ?
						500 :
						250;
			}
	...
	public void setCurrentPager(int currentTab) {
		...
		mTabsContainer.getChildAt(currentTab)
					  .performClick();
	}
		...
		if (tipView != null) {
			...
			if (!mIconVisible) {
				...
				setMsgMargin(
						position,
						0,
						mIconGravity == Gravity.LEFT || mIconGravity == Gravity.RIGHT ?
								4 :
								0
							);
			}
	...
	public void setMsgMargin(int position, float leftPadding, float bottomPadding) {
		...
		if (tipView != null) {
			...
			float textWidth = mTextPaint.measureText(tv_tab_title.getText()
																 .toString());
			...
			if (mIconVisible) {
				if (iconH <= 0) {
					iconH = mContext.getResources()
									.getDrawable(mTabEntitys.get(position)
															.getTabSelectedIcon())
									.getIntrinsicHeight();
				}
				margin = mIconMargin;
			...
			if (mIconGravity == Gravity.TOP || mIconGravity == Gravity.BOTTOM) {
				...
				lp.leftMargin = dp2px(leftPadding);
				lp.topMargin = mHeight > 0 ?
						(int) (mHeight - Math.max(
								textHeight,
								iconH
												 )) / 2 - dp2px(bottomPadding) :
						dp2px(bottomPadding);
			}
	...
	protected int sp2px(float sp) {
		...
		final float scale = this.mContext.getResources()
