 ...
 */
public class IrregularLayoutManager
	...
	public IrregularLayoutManager(boolean center) {
	...
	public IrregularLayoutManager(int groupSize, boolean center) {
		...
		for (int i = 0; i < getItemCount(); i++) {
			...
			if (isItemInFirstLine(i)) {
				int offsetInLine = i < firstLineSize ?
						...
						i % mGroupSize;
				item.set(
						mGravityOffset + offsetInLine * itemWidth,
						offsetHeight,
				...
				item.set(
						mGravityOffset + offsetInLine * itemWidth + itemWidth / 2,
