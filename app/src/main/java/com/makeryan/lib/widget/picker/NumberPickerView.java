package com.makeryan.lib.widget.picker;
...
public class NumberPickerView
	...
	private static final int DEFAULT_DIVIDER_COLOR = 0XFFFF0000;
	...
	private void initAttr(Context context, AttributeSet attrs) {
		...
		for (int i = 0; i < n; i++) {
			...
			if (attr == R.styleable.NumberPickerView_npv_ShowCount) {
				...
				mDividerMarginL = a.getDimensionPixelSize(
						attr,
				...
				mMarginStartOfHint = a.getDimensionPixelSize(
						...
						dp2px(
								context,
								DEFAULT_MARGIN_START_OF_HINT_DP
							 )
															);
	...
	private void init(Context context) {
		...
		mMiniVelocityFling = ViewConfiguration.get(getContext())
											  .getScaledMinimumFlingVelocity();
		mScaledTouchSlop = ViewConfiguration.get(getContext())
											.getScaledTouchSlop();
		if (mTextSizeNormal == 0) {
			mTextSizeNormal = sp2px(
					context,
					DEFAULT_TEXT_SIZE_NORMAL_SP
								   );
		}
		if (mTextSizeSelected == 0) {
			mTextSizeSelected = sp2px(
					context,
					DEFAULT_TEXT_SIZE_SELECTED_SP
									 );
		}
		...
		if (mMarginStartOfHint == 0) {
			mMarginStartOfHint = dp2px(
					context,
					DEFAULT_MARGIN_START_OF_HINT_DP
									  );
		}
		if (mMarginEndOfHint == 0) {
			mMarginEndOfHint = dp2px(
					context,
					DEFAULT_MARGIN_END_OF_HINT_DP
									);
		}
	...
	private void respondPickedValueChangedInScrolling(int oldVal, int newVal) {
		...
		mOnValueChangeListenerInScrolling.onValueChangeInScrolling(
				this,
				oldVal,
				newVal
																  );
	...
	public void setDisplayedValues(String[] newDisplayedValues, boolean needRefresh) {
		...
		setDisplayedValuesAndPickedIndex(
				newDisplayedValues,
				0,
				needRefresh
										);
	 ...
	 *
	 * @param toValue
	 * 		the value you want picker to scroll to
	 */
	 ...
	 * 		the value you want picker to scroll to
	 * @param needRespond
	 * 		set if you want picker to respond onValueChange listener
	 */
	...
	/**
	 * @param fromValue
	 * 		need to set the fromValue, can be greater than mMaxValue or less than mMinValue
	 * @param toValue
	 * 		the value you want picker to scroll to
	 * @param needRespond
	 * 		need Respond to the ValueChange callback When Scrolling, default is false
	 */
	 ...
	 * 		currPicked value
	 * @param respondChange
	 * 		if want to respond onchange callbacks
	 */
	...
	/**
	 * @param deltaIndex
	 * 		the delta index it will scroll by
	 * @param needRespond
	 * 		need Respond to the ValueChange callback When Scrolling, default is false
	 */
	...
	public void setNpv_MinValue(int minValue) {
	...
	public void setNpv_MaxValue(int maxValue) {
	 ...
	 *
	 * @param friction
	 * 		default is ViewConfiguration.get(mContext).getScrollFriction()
	...
	public void setFriction(float friction) {
		...
		mFriction = ViewConfiguration.get(getContext())
									 .getScrollFriction() / friction;
	}
	...
	private void onScrollStateChange(int scrollState) {
		...
		if (mOnScrollListener != null) {
			mOnScrollListener.onScrollStateChange(
					this,
					scrollState
												 );
		}
	...
	private int measureWidth(int measureSpec) {
		...
		if (specMode == MeasureSpec.EXACTLY) {
			...
			int marginOfHint = Math.max(
					mWidthOfHintText,
					mWidthOfAlterHint
									   ) == 0 ?
					0 :
					mMarginEndOfHint;
			int gapOfHint = Math.max(
					mWidthOfHintText,
					mWidthOfAlterHint
									) == 0 ?
					0 :
	...
	private void drawContent(Canvas canvas) {
		...
		for (int i = 0; i < mShowCount + 1; i++) {
			...
			if (0 <= index && index < getOneRecycleSize()) {
				CharSequence str = mDisplayedValues[index];
				if (mTextEllipsize != null) {
					str = TextUtils.ellipsize(
							str,
							mPaintText,
							getWidth() - 2 * mItemPaddingHorizontal,
							getEllipsizeType()
											 );
				}
				canvas.drawText(
	...
	private void drawLine(Canvas canvas) {
		...
		if (mShowDivider) {
			canvas.drawLine(
					getPaddingLeft() + mDividerMarginL,
					dividerY0,
					mViewWidth - getPaddingRight() - mDividerMarginR,
					dividerY0,
					mPaintDivider
						   );
			canvas.drawLine(
	...
	private void drawHint(Canvas canvas) {
		...
		canvas.drawText(
				...
				(dividerY0 + dividerY1) / 2 + mTextSizeHintCenterYOffset,
				mPaintHint
					   );
	}
	...
	private void updateMaxWidthOfDisplayedValues() {
		...
		mWidthOfAlterHint = getTextWidth(
				mAlterHint,
				mPaintText
										);
	...
	private int dp2px(Context context, float dpValue) {
		...
		final float densityScale = context.getResources()
