...
public class VisibleDelegate {
	...
	public void onActivityCreated(@Nullable Bundle savedInstanceState) {
		...
		if (!mInvisibleWhenLeave && !mSupportFragment.isHidden() && (mSupportFragment.getUserVisibleHint() || mFixStatePagerAdapter)) {
			if ((mSupportFragment.getParentFragment() != null && isFragmentVisible(mSupportFragment.getParentFragment())) || mSupportFragment.getParentFragment() == null) {
				mNeedDispatch = false;
				...
				if (childFragments != null) {
					for (Fragment child : childFragments) {
						if (child instanceof SupportFragment && !child.isHidden() && child.getUserVisibleHint()) {
							((SupportFragment) child).getVisibleDelegate()
