package com.makeryan.lib.fragment.fragmentation.debug;
...
public class DebugHierarchyViewContainer
	...
	public void bindFragmentRecords(List<DebugFragmentRecord> fragmentRecords) {
		...
		DebugHierarchyViewContainer.this.setView(
				fragmentRecords,
				0,
				null
												);
	...
	private void setView(final List<DebugFragmentRecord> fragmentRecordList, final int hierarchy, final TextView tvItem) {
		...
		for (int i = fragmentRecordList.size() - 1; i >= 0; i--) {
			...
			if (childFragmentRecord != null && childFragmentRecord.size() > 0) {
				...
				childTvItem.setPadding(
						childTvItem.getPaddingLeft() + mPadding,
						0,
						mPadding,
						0
									  );
			...
			if (tvItem == null) {
				...
				mLinearLayout.addView(
						childTvItem,
						mLinearLayout.indexOfChild(tvItem) + 1
									 );
			}
		}
	}
	...
	private TextView getTextView(DebugFragmentRecord fragmentRecord, int hierarchy) {
		...
		tvItem.setPadding(
				(int) (mPadding + hierarchy * mPadding * 1.5),
				0,
				mPadding,
				0
						 );
