...
import com.makeryan.lib.fragment.fragmentation.debug.DebugFragmentRecord;
import com.makeryan.lib.fragment.fragmentation.debug.DebugHierarchyViewContainer;
...
class FragmentationDelegate {
	...
	void loadRootTransaction(FragmentManager fragmentManager, int containerId, SupportFragment to) {
		...
		SupportFragment fragment = findFragmentByRoot(
				fragmentManager,
				to.getClass()
				  .getName()
													 );
	...
	void loadMultipleRootTransaction(FragmentManager fragmentManager, int containerId, int showPosition, SupportFragment... tos) {
		...
		fragmentManager = checkFragmentManager(
				fragmentManager,
				null
											  );
	...
	void dispatchStartTransaction(FragmentManager fragmentManager, SupportFragment from, SupportFragment to, int requestCode, int launchMode, int type) {
		...
		if ((from != null && from.isRemoving())) {
			Log.e(
					TAG,
					from.getClass()
						.getSimpleName() + " is poped, maybe you want to call startWithPop()!"
				 );
			return;
		}
			...
			case TYPE_ADD_RESULT:
				start(
						...
						transactionRecord == null ?
								null :
								transactionRecord.sharedElementList
					 );
				break;
	...
	void replaceTransaction(FragmentManager fragmentManager, int containerId, SupportFragment to, boolean addToBack) {
		...
		if (addToBack) {
			ft.addToBackStack(to.getClass()
								.getName());
		}
		Bundle bundle = to.getArguments();
		bundle.putBoolean(
				FRAGMENTATION_ARG_IS_ROOT,
				true
						 );
		supportCommit(
				fragmentManager,
	...
	void start(FragmentManager fragmentManager, SupportFragment from, SupportFragment to, String toFragmentTag, ArrayList<TransactionRecord.SharedElement> sharedElementList) {
		...
		if (from == null) {
			ft.add(
					bundle.getInt(FRAGMENTATION_ARG_CONTAINER),
					to,
					toFragmentTag
				  );
			bundle.putBoolean(
	...
	void startWithPop(final FragmentManager fragmentManager, SupportFragment from, SupportFragment to, String toFragmentTag) {
		...
		if (from.isHidden()) {
			Log.e(
					TAG,
					from.getClass()
						.getSimpleName() + " is hidden, " + "the transaction of startWithPop() is invalid!"
				 );
			return;
		}
	...
	<T extends SupportFragment> T findStackFragment(Class<T> fragmentClass, String toFragmentTag, FragmentManager fragmentManager) {
		...
		if (toFragmentTag == null) {
			...
			for (int i = sizeChildFrgList - 1; i >= 0; i--) {
				...
				if (brotherFragment instanceof SupportFragment && brotherFragment.getClass()
																				 .getName()
	...
	private boolean handleLaunchMode(FragmentManager fragmentManager, SupportFragment toFragment, String toFragmentTag, int launchMode) {
		...
		Fragment stackToFragment = findStackFragment(
				toFragment.getClass(),
				toFragmentTag,
				fragmentManager
													);
		...
		if (launchMode == SupportFragment.SINGLETOP) {
			// 在栈顶
			if (toFragment == topFragment || toFragment.getClass()
													   .getName()
													   .equals(topFragment.getClass()
																		  .getName())) {
				handleNewBundle(
						toFragment,
						stackToFragment
	...
	void popTo(String fragmentTag, boolean includeSelf, Runnable afterPopTransactionRunnable, FragmentManager fragmentManager) {
		...
		if (targetFragment == null) {
			Log.e(
					TAG,
					"Pop failure! Can't find FragmentTag:" + fragmentTag + " in the FragmentManager's Stack."
				 );
			return;
		}
				...
				if (preViewGroup != null) {
					...
					restoreView(
							...
							toFragment == null ?
									BUFFER_TIME :
									Math.max(
											fromFragment.getEnterAnimDuration(),
											fromFragment.getPopExitAnimDuration()
											) + BUFFER_TIME
							   );
				}
			}
	...
	private FragmentManager checkFragmentManager(FragmentManager fragmentManager, Fragment from) {
		...
		if (fragmentManager == null) {
			if (mPopToTempFragmentManager == null) {
				...
				Log.e(
						TAG,
						fromName + "'s FragmentManager is null, " + " Please check if " + fromName + " is destroyed!"
					 );
				return null;
			}
	...
	void showFragmentStackHierarchyView() {
		...
		mStackDialog = new AlertDialog.Builder(mActivity).setTitle("栈视图")
														 .setView(container)
														 .setPositiveButton(
																 "关闭",
																 null
	...
	void logFragmentRecords(String tag) {
		...
		for (int i = fragmentRecordList.size() - 1; i >= 0; i--) {
			...
			if (i == fragmentRecordList.size() - 1) {
				...
				if (i == 0) {
					sb.append("\t栈顶\t\t\t")
					  .append(fragmentRecord.fragmentName)
					  .append("\n");
					sb.append("═══════════════════════════════════════════════════════════════════════════════════");
				} else {
					sb.append("\t栈顶\t\t\t")
				...
				sb.append("\t栈底\t\t\t")
				  ...
				  .append("\n\n");
				processChildLog(
						fragmentRecord.childFragmentRecord,
						sb,
						1
							   );
				sb.append("═══════════════════════════════════════════════════════════════════════════════════");
	...
	private List<DebugFragmentRecord> getFragmentRecords() {
		...
		List<Fragment> fragmentList = mActivity.getSupportFragmentManager()
											   .getFragments();
		...
		for (Fragment fragment : fragmentList) {
			...
			fragmentRecordList.add(new DebugFragmentRecord(
					fragment.getClass()
							.getSimpleName(),
	...
	private void processChildLog(List<DebugFragmentRecord> fragmentRecordList, StringBuilder sb, int childHierarchy) {
		...
		for (int j = 0; j < fragmentRecordList.size(); j++) {
			...
			if (j == 0) {
				sb.append("\t子栈顶\t\t")
				  .append(childFragmentRecord.fragmentName)
				  .append("\n\n");
				...
				sb.append("\t子栈底\t\t")
				  ...
				  .append("\n\n");
				processChildLog(
						childFragmentRecord.childFragmentRecord,
						sb,
						++childHierarchy
							   );
				return;
	...
	private List<DebugFragmentRecord> getChildFragmentRecords(Fragment parentFragment) {
		...
		for (int i = fragmentList.size() - 1; i >= 0; i--) {
			Fragment fragment = fragmentList.get(i);
			if (fragment != null) {
				fragmentRecords.add(new DebugFragmentRecord(
						fragment.getClass()
								.getSimpleName(),
						getChildFragmentRecords(fragment)
				));
