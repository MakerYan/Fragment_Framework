 ...
 */
public class Insert<TModel extends Model>
	...
	public Insert<TModel> asColumns() {
		...
		columns(FlowManager.getModelAdapter(getTable())
	 ...
	 *
	 * @param conditions
	 * 		The conditions that we use to fill the columns and values of this INSERT
	 */
	...
	public Insert<TModel> columnValues(ConditionGroup conditionGroup) {
		...
		for (int i = 0; i < size; i++) {
			SQLCondition condition = conditionGroup.getConditions()
												   .get(i);
			columns[i] = condition.columnName();
			values[i] = condition.value();
		}
	 ...
	 *
	 * @param selectFrom
	 * 		The from that is continuation of {@link Select}.
	 */
	...
	public String getQuery() {
		...
		if (conflictAction != null && !conflictAction.equals(ConflictAction.NONE)) {
			queryBuilder.append("OR")
						.appendSpaceSeparated(conflictAction);
		}
