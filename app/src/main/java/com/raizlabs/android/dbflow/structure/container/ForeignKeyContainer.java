 ...
 */
public class ForeignKeyContainer<TModel extends Model>
		extends SimpleModelContainer<TModel, Map<String, Object>> {
	 ...
	 *
	 * @param table
	 * 		The table to associate the container with
	 */
	...
	public BaseModelContainer getInstance(Object inValue, Class<? extends Model> columnClass) {
		...
		if (inValue instanceof ModelContainer) {
			...
			return new ForeignKeyContainer(
					columnClass,
	...
	public TModel load() {
		...
		if (relationshipModel == null && getData() != null) {
			relationshipModel = new Select().from(modelAdapter.getModelClass())
											.where(modelContainerAdapter.getPrimaryConditionClause(this))
											.querySingle();
		}
	...
	private static class InvalidMethodCallException
