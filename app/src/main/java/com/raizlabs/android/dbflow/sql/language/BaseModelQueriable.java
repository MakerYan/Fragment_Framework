...
public abstract class BaseModelQueriable<TModel extends Model>
	 ...
	 *
	 * @param table
	 * 		the table that belongs to this query.
	 */
	...
	public CursorResult<TModel> queryResults() {
		...
		return new CursorResult<>(
				getRetrievalAdapter().getModelClass(),
				query()
		);
	}
	...
	public List<TModel> queryList(DatabaseWrapper wrapper) {
		...
		List<TModel> list = getRetrievalAdapter().getListModelLoader()
												 .load(
														 wrapper,
														 query
													  );
	...
	public FlowCursorList<TModel> cursorList() {
		...
		return new FlowCursorList.Builder<>(getTable()).modelQueriable(this)
													   .build();
	}
	...
	public FlowQueryList<TModel> flowQueryList() {
		...
		return new FlowQueryList.Builder<>(getTable()).modelQueriable(this)
													  .build();
	}
	...
	public <QueryClass extends BaseQueryModel> List<QueryClass> queryCustomList(Class<QueryClass> queryModelClass) {
		...
		return FlowManager.getQueryModelAdapter(queryModelClass)
						  .getListModelLoader()
						  .load(query);
	}
