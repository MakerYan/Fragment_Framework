...
public abstract class RetrievalAdapter<TModel extends Model, TTable extends Model> {
	 ...
	 *
	 * @param model
	 * 		The model to assign cursor data to
	 * @param cursor
	 * 		The cursor to load into the model
	 */
	public abstract void loadFromCursor(Cursor cursor, TModel model);

	/**
	 * @param model
	 * 		The model to query values from
	 *
	...
	public boolean exists(TModel model) {
		...
		return exists(
				model,
				FlowManager.getDatabaseForTable(getModelClass())
						   .getWritableDatabase()
					 );
	}
