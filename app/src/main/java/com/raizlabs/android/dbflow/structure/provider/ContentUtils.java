...
public class ContentUtils {
	 ...
	 *
	 * @param authority
	 * 		The authority for a {@link ContentProvider}
	 * @param paths
	 ...
	 *
	 * @param baseContentUri
	 * 		The base content URI for a {@link ContentProvider}
	 * @param authority
	 * 		The authority for a {@link ContentProvider}
	 * @param paths
	 * 		The list of paths to append.
	...
	public static Uri buildUri(String baseContentUri, String authority, String... paths) {
		...
		Uri.Builder builder = Uri.parse(baseContentUri + authority)
								 .buildUpon();
	 ...
	 *
	 * @param insertUri
	 * 		A {@link Uri} from the {@link ContentProvider} class definition.
	 * @param model
	 * 		The model to insert.
	 * @param <TableClass>
	 * 		The class that implemets {@link Model}
	 *
	 ...
	 * 		The content resolver to use (if different from {@link FlowManager#getContext()})
	 * @param insertUri
	 * 		A {@link Uri} from the {@link ContentProvider} class definition.
	 * @param model
	 * 		The model to insert.
	...
	public static <TableClass extends Model> int bulkInsert(ContentResolver contentResolver, Uri bulkInsertUri, Class<TableClass> table, List<TableClass> models) {
		...
		ContentValues[] contentValues = new ContentValues[models == null ?
				0 :
				models.size()];
	 ...
	 *
	 * @param updateUri
	 * 		A {@link Uri} from the {@link ContentProvider}
	 * @param model
	 * 		A model to update
	 * @param <TableClass>
	 * 		The class that implements {@link Model}
	...
	public static <TableClass extends Model> int update(ContentResolver contentResolver, Uri updateUri, TableClass model) {
		...
		if (count == 0) {
			FlowLog.log(
					FlowLog.Level.W,
					"Updated failed of: " + model.getClass()
					   );
		}
		return count;
	}
	...
	public static <TableClass extends Model> int delete(ContentResolver contentResolver, Uri deleteUri, TableClass model) {
		...
		if (count > 0) {
			...
			FlowLog.log(
					FlowLog.Level.W,
					"A delete on " + model.getClass() + " within the ContentResolver appeared to fail."
					   );
		}
		return count;
	}
	 ...
	 * 		The set of {@link Condition} to query the content provider.
	 * @param orderBy
	 * 		The order by clause without the ORDER BY
	 * @param columns
	...
	public static <TableClass extends Model> TableClass querySingle(ContentResolver contentResolver, Uri queryUri, Class<TableClass> table, ConditionGroup whereConditions, String orderBy, String... columns) {
		...
		List<TableClass> list = queryList(
				contentResolver,
				queryUri,
				table,
				whereConditions,
