...
public class FlowCursorList<TModel extends Model>
	 ...
	 *
	 * @param cacheModels
	 * 		For every call to {@link #getItem(long)}, we want to keep a reference to it so
	 ...
	 *
	 * @param cacheModels
	 * 		true, will cache models. If false, any and future caching is cleared.
	 *
	 ...
	 *
	 * @param position
	 * 		The row number in the {@link Cursor} to look at
	 *
	...
	public TModel getItem(long position) {
		...
		if (cacheModels) {
			...
			if (model == null && cursor != null && cursor.moveToPosition((int) position)) {
				model = modelAdapter.getSingleModelLoader()
									.convertToData(
											cursor,
											null,
											false
	 ...
	 * 		when the cursor backing this list is closed.
	...
	private void warnEmptyCursor() {
		...
		if (cursor == null) {
			FlowLog.log(
					FlowLog.Level.W,
					"Cursor was null for FlowCursorList"
					   );
		}
	}
	...
	public Builder<TModel> newBuilder() {
		...
		return new Builder<>(table).modelQueriable(modelQueriable)
								   .cursor(cursor)
								   .cacheSize(cacheSize)
