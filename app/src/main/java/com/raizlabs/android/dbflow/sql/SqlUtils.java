...
public class SqlUtils {
	 ...
	 * @param adapter
	 * 		The adapter to use thats either a {@link ModelAdapter} or {@link ModelContainerAdapter}
	 * 		to handle interactions.
	 * @param modelAdapter
	 * 		The actual {@link ModelAdapter} associated with the {@link ModelClass}/
	 * @param action
	 * 		The {@link Action} that occured.
	 * @param <ModelClass>
	 * 		The original model class.
	 * @param <TableClass>
	 * 		The class of the adapter that we use the model from.
	 * @param <AdapterClass>
	 * 		The class of the adapter, which is either a {@link ModelAdapter} or {@link ModelContainerAdapter}
	 */
	@SuppressWarnings("unchecked")
	public static <ModelClass extends Model, TableClass extends Model, AdapterClass extends RetrievalAdapter & InternalAdapter> void notifyModelChanged(TableClass model, AdapterClass adapter, ModelAdapter<ModelClass> modelAdapter, Action action) {

		if (FlowContentObserver.shouldNotify()) {
			notifyModelChanged(
					modelAdapter.getModelClass(),
					action,
	 ...
	 * 		The action to use.
	 * @param conditions
	 * 		The set of key-value {@link SQLCondition} to construct into a uri.
	 *
	 * @return The {@link Uri}.
	 */
		...
		if (conditions != null) {
			for (SQLCondition condition : conditions) {
				uriBuilder.appendQueryParameter(
						...
						Uri.encode(String.valueOf(condition.value()))
											   );
			}
		}
	...
	public static Uri getNotificationUri(Class<? extends Model> modelClass, Action action, String notifyKey, Object notifyValue) {
		...
		if (StringUtils.isNotNullOrEmpty(notifyKey)) {
			condition = Condition.column(new NameAlias.Builder(notifyKey).build())
								 .value(notifyValue);
		}
		return getNotificationUri(
				modelClass,
				action,
				new SQLCondition[]{condition}
								 );
	}
	 ...
	 *
	 * @param mOnTable
	 * 		The table that this trigger runs on
	 * @param triggerName
	...
	public static <ModelClass extends Model> void dropTrigger(Class<ModelClass> mOnTable, String triggerName) {
		...
		QueryBuilder queryBuilder = new QueryBuilder("DROP TRIGGER IF EXISTS ").append(triggerName);
		FlowManager.getDatabaseForTable(mOnTable)
				   .getWritableDatabase()
				   .execSQL(queryBuilder.getQuery());
	}
	 ...
	 *
	 * @param mOnTable
	 * 		The table that this index runs on
	 * @param indexName
	...
	public static <ModelClass extends Model> void dropIndex(DatabaseWrapper databaseWrapper, String indexName) {
		...
		QueryBuilder queryBuilder = new QueryBuilder("DROP INDEX IF EXISTS ").append(QueryBuilder.quoteIfNeeded(indexName));
	...
	public static <ModelClass extends Model> void dropIndex(Class<ModelClass> onTable, String indexName) {
		...
		dropIndex(
				FlowManager.getDatabaseForTable(onTable)
						   .getWritableDatabase(),
				indexName
				 );
	}
	 ...
	 *
	 * @param contentValues
	 * 		The content values to convert.
	 * @param conditionGroup
	 * 		The group to put them into as {@link Condition}.
	 */
