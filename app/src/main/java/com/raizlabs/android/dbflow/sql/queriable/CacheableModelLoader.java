 ...
 * is true.
 */
public class CacheableModelLoader<TModel extends Model>
	...
	public TModel convertToData(@NonNull Cursor cursor, @Nullable TModel data, boolean moveToFirst) {
		...
		if (!moveToFirst || cursor.moveToFirst()) {
			Object[] values = getModelAdapter().getCachingColumnValuesFromCursor(
					new Object[getModelAdapter().getCachingColumns().length],
					cursor
																				);
			...
			if (model == null) {
				...
				getModelAdapter().loadFromCursor(
						cursor,
						model
												);
				getModelCache().addModel(
						getModelAdapter().getCachingId(values),
						model
										);
			} else {
				getModelAdapter().reloadRelationships(
						model,
						cursor
													 );
			}
