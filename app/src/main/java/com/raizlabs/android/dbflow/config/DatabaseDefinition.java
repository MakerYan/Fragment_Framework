...
public abstract class DatabaseDefinition {
	...
	public DatabaseDefinition() {
		...
		if (databaseConfig != null) {
			...
			Collection<TableConfig> tableConfigCollection = databaseConfig.tableConfigMap()
																		  .values();
		...
		if (databaseConfig == null || databaseConfig.transactionManagerCreator() == null) {
			...
			transactionManager = databaseConfig.transactionManagerCreator()
											   .createManager(this);
		}
	}
	...
	/**
	 * @param tableName
	 * 		The name of the table in this db.
	 *
	...
	/**
	 * @param table
	 * 		The table that has a {@link ModelContainer} annotation.
	 *
	...
	/**
	 * @param table
	 * 		the VIEW class to retrieve the ModelViewAdapter from.
	 *
	...
	public synchronized OpenHelper getHelper() {
		...
		if (openHelper == null) {
			...
			if (config == null || config.helperCreator() == null) {
				openHelper = new FlowSQLiteOpenHelper(
						this,
						helperListener
				);
			} else {
	 ...
	 * 		if {@link Database#backupEnabled()}
