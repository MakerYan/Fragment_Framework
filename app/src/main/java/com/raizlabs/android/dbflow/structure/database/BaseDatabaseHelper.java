...
public class BaseDatabaseHelper {
	...
	protected void checkForeignKeySupport(DatabaseWrapper database) {
		...
		if (databaseDefinition.isForeignKeysSupported()) {
			...
			FlowLog.log(
					FlowLog.Level.I,
					"Foreign Keys supported. Enabling foreign key features."
					   );
		}
	}
	...
	protected void executeCreations(final DatabaseWrapper database) {
		...
		try {
			...
			for (ModelViewAdapter modelView : modelViews) {
				QueryBuilder queryBuilder = new QueryBuilder().append("CREATE VIEW IF NOT EXISTS")
	...
	protected void executeMigrations(final DatabaseWrapper db, final int oldVersion, final int newVersion) {
		...
		try {
			final List<String> files = Arrays.asList(FlowManager.getContext()
																.getAssets()
			...
			for (String file : files) {
				try {
					final Integer version = Integer.valueOf(file.replace(
							".sql",
							""
																		));
					...
					FlowLog.log(
							FlowLog.Level.W,
							"Skipping invalidly named file: " + file,
							e
							   );
				}
			}
				...
				for (int i = curVersion; i <= newVersion; i++) {
					...
					if (migrationFiles != null) {
						for (String migrationFile : migrationFiles) {
							...
							FlowLog.log(
									FlowLog.Level.I,
									migrationFile + " executed successfully."
									   );
						}
					}
					...
					if (migrationMap != null) {
						...
						if (migrationsList != null) {
							for (Migration migration : migrationsList) {
								...
								FlowLog.log(
										FlowLog.Level.I,
										migration.getClass() + " executed successfully."
										   );
							}
						}
					}
	 ...
	 *
	 * @param db
	 * 		The database to run it on
	 * @param file
	 * 		the file name in assets/migrations that we read from
	 */
	private void executeSqlScript(DatabaseWrapper db, String file) {

		try {
			final InputStream input = FlowManager.getContext()
												 .getAssets()
												 .open(MIGRATION_PATH + "/" + getDatabaseDefinition().getDatabaseName() + "/" + file);
			...
			while ((line = reader.readLine()) != null) {
				...
				if (isEndOfQuery) {
					line = line.substring(
							0,
							line.length() - querySuffix.length()
										 );
				}
				query.append(" ")
					 .append(line);
