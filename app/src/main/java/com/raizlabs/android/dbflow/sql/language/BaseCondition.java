...
public abstract class BaseCondition
	 ...
	 *
	 * @param value
	 * 		The value of the column in Model format.
	 * @param appendInnerQueryParenthesis
	 * 		if its a {@link BaseModelQueriable} and an inner query value
		...
		if (value == null) {
			...
			if (value instanceof Number) {
				...
				if (appendInnerQueryParenthesis && value instanceof BaseModelQueriable) {
					stringVal = String.format(
							"(%1s)",
							((BaseModelQueriable) value).getQuery()
														.trim()
											 );
	 ...
	 * 		The text to join the text with.
	 * @param tokens
	 * 		an {@link Iterable} of objects to be joined. Strings will be formed from
	 * 		the objects by calling {@link #convertValueToString(Object, boolean)}.
	 *
	 * @return A joined string
	 */
