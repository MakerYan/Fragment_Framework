 ...
 */
public class ModelLruCache<TModel extends Model>
		extends ModelCache<TModel, LruCache<Long, TModel>> {

	/**
	 * @param size
	 * 		The size, if less than or equal to 0 we set it to {@link Table#DEFAULT_CACHE_SIZE}.
	 */
	...
	public void addModel(Object id, TModel model) {
		...
		if (id instanceof Number) {
			...
			throw new IllegalArgumentException("A ModelLruCache must use an id that can cast to" + "a Number to convert it into a long");
		}
	}
	...
	public TModel removeModel(Object id) {
		...
		if (id instanceof Number) {
			...
			throw new IllegalArgumentException("A ModelLruCache uses an id that can cast to" + "a Number to convert it into a long");
		}
		return model;
	}
