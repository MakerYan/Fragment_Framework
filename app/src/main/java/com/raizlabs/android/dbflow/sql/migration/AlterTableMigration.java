...
public class AlterTableMigration<TModel extends Model>
	...
	public final void migrate(DatabaseWrapper database) {
		...
		if (columnDefinitions != null) {
			...
			Cursor cursorToCheckColumnFor = SQLite.select()
												  .from(table)
												  .limit(0)
												  .query(database);
			if (cursorToCheckColumnFor != null) {
				try {
					sql = new QueryBuilder(sql).append(tableName)
											   .toString();
	 ...
	 *
	 * @param oldName
	 * 		The new name to call the table.
	 *
	...
	public AlterTableMigration<TModel> renameFrom(@NonNull String oldName) {
		...
		renameQuery = new QueryBuilder().append(" RENAME")
	 ...
	 *
	 * @param sqLiteType
	 * 		The type of column represented in the DB.
	 * @param columnName
	...
	public AlterTableMigration<TModel> addColumn(@NonNull SQLiteType sqLiteType, @NonNull String columnName) {
		...
		QueryBuilder queryBuilder = new QueryBuilder().append(QueryBuilder.quoteIfNeeded(columnName))
													  .appendSpace()
	 ...
	 *
	 * @param sqLiteType
	 * 		The type of column that pertains to an {@link SQLiteType}
	 * @param columnName
	 * 		The name of the column to add. Use the "$Table" class for the specified table.
	 * @param referenceClause
	 * 		The clause of the references that this foreign key points to.
	 *
	 * @return This instance
	 */
	...
	public List<String> getColumnDefinitions() {
		...
		String sql = new QueryBuilder(getAlterTableQueryBuilder()).append(FlowManager.getTableName(table))
																  .toString();
		List<String> columnDefinitions = new ArrayList<>();

		if (this.columnDefinitions != null) {
			for (QueryBuilder columnDefinition : this.columnDefinitions) {
				QueryBuilder queryBuilder = new QueryBuilder(sql).appendSpaceSeparated("ADD COLUMN")
																 .append(columnDefinition.getQuery());
	...
	public QueryBuilder getAlterTableQueryBuilder() {
		...
		if (query == null) {
			query = new QueryBuilder().append("ALTER")
