...
public class DBBatchSaveQueue
	 ...
	 *
	 * @param time
	 * 		The time, in millis that queue automatically checks for leftover {@link Model} in this queue.
	 */
	 ...
	 * 		The listener to get notified when changes are successful.
	 */
	...
	public void run() {
		...
		while (true) {
			...
			if (tmpModels.size() > 0) {
				databaseDefinition.beginTransactionAsync(new ProcessModelTransaction.Builder(modelSaver).addAll(tmpModels)
																										.build())
			...
			try {
				...
				FlowLog.log(
						FlowLog.Level.I,
						"DBRequestQueue Batch interrupted to start saving"
						   );
			}
