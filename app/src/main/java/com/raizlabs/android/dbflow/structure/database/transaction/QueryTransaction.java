...
public class QueryTransaction<TResult extends Model>
	 ...
	 *
	 * @param <TResult>
	 * 		The result that we got from querying.
	 */
	public interface QueryResultCallback<TResult extends Model> {
		 ...
		 *
		 * @param transaction
		 * 		The transaction that ran.
		 * @param tResult
		 * 		The result of the query. Use this object to get data that you need.
		 */
	...
	public interface QueryResultListCallback<TResult extends Model> {
		 ...
		 * 		The transaction that ran.
		 * @param tResult
		 * 		The {@link List} result of the query.
		 */
	...
	public void execute(DatabaseWrapper databaseWrapper) {
		...
		if (queryResultCallback != null) {
			if (runResultCallbacksOnSameThread) {
				queryResultCallback.onQueryResult(
						this,
						cursorResult
												 );
			} else {
		...
		if (queryResultListCallback != null) {
			...
			if (runResultCallbacksOnSameThread) {
				queryResultListCallback.onListQueryResult(
						this,
						resultList
														 );
			} else {
		...
		if (queryResultSingleCallback != null) {
			...
			if (runResultCallbacksOnSameThread) {
				queryResultSingleCallback.onSingleQueryResult(
						this,
						result
															 );
			} else {
