...
public class PropertyFactory {
	 ...
	 *
	 * @param type
	 * 		The object with value to use.
	 * @param <T>
	 * 		The parameter of its type.
	 *
	...
	public static <T> Property<T> from(@Nullable T type) {
		...
		return new Property<>(
				null,
				NameAlias.rawBuilder(Condition.convertValueToString(type))
						 .build()
		);
	}
	 ...
	 *
	 * @param queriable
	 * 		The queriable to use and evaulated into a query.
	 * @param <TModel>
	 * 		The model class of the query.
	 *
	...
	public static <TModel extends Model> Property<TModel> from(@NonNull ModelQueriable<TModel> queriable) {
		...
		return from(
				queriable.getTable(),
				"(" + String.valueOf(queriable.getQuery())
							.trim() + ")"
				   );
	}
	 ...
	 * 		The string representation of the object you wish to use.
	 * @param <T>
	 * 		The parameter of its type.
	 *
	 * @return A new property with its type.
	...
	public static <T> Property<T> from(@Nullable Class<T> type, String stringRepresentation) {
		...
		return new Property<>(
				null,
				NameAlias.rawBuilder(stringRepresentation)
						 .build()
		);
	}
}
