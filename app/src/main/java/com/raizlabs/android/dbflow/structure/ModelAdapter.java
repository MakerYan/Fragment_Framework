 ...
 */
public abstract class ModelAdapter<TModel extends Model>
		extends InstanceAdapter<TModel, TModel>
	...
	public DatabaseStatement getInsertStatement() {
		...
		if (insertStatement == null) {
			insertStatement = getInsertStatement(FlowManager.getDatabaseForTable(getModelClass())
															.getWritableDatabase());
		}
	 ...
	 * 		The database used to do an insert statement.
	 *
	...
	public DatabaseStatement getCompiledStatement() {
		...
		if (compiledStatement == null) {
			compiledStatement = getCompiledStatement(FlowManager.getDatabaseForTable(getModelClass())
																.getWritableDatabase());
		}
	 ...
	 *
	 * @param model
	 * 		The model object to store the key
	 * @param id
	 * 		The key to store
	 */
	@Override
	...
	public Number getAutoIncrementingId(TModel model) {
		...
		throw new InvalidDBConfiguration(String.format(
				"This method may have been called in error. The model class %1s must contain" + "a single primary key (if used in a ModelCache, this method may be called)",
				getModelClass()
													  ));
	}
	...
	public String getAutoIncrementingColumnName() {
		...
		throw new InvalidDBConfiguration(String.format(
				"This method may have been called in error. The model class %1s must contain " + "an autoincrementing or single int/long primary key (if used in a ModelCache, this method may be called)",
				getModelClass()
													  ));
	}
	 ...
	 *
	 * @param inValues
	 * 		The reusable array of values to populate.
	 * @param cursor
	 * 		The cursor to load from.
	 *
	...
	public Object getCachingId(@NonNull TModel model) {
		...
		return getCachingId(getCachingColumnValuesFromModel(
				new Object[getCachingColumns().length],
				model
														   ));
	}
	...
	public IMultiKeyCacheConverter<?> getCacheConverter() {
		...
		throw new InvalidDBConfiguration("For multiple primary keys, a public static IMultiKeyCacheConverter field must" + "be  marked with @MultiCacheField in the corresponding model class. The resulting key" + "must be a unique combination of the multiple keys, otherwise inconsistencies may occur.");
	}
	...
	private void throwCachingError() {
		...
		throw new InvalidDBConfiguration(String.format(
				"This method may have been called in error. The model class %1s must contain" + "an auto-incrementing or at least one primary key (if used in a ModelCache, this method may be called)",
				getModelClass()
													  ));
	}
	...
	private void throwSingleCachingError() {
		...
		throw new InvalidDBConfiguration(String.format(
				"This method may have been called in error. The model class %1s must contain" + "an auto-incrementing or one primary key (if used in a ModelCache, this method may be called)",
				getModelClass()
													  ));
	}
}
