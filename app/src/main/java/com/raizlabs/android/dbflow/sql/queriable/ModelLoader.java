...
public abstract class ModelLoader<TModel extends Model, TReturn> {
	...
	public TReturn load(String query) {
		...
		return load(
				getDatabaseDefinition().getWritableDatabase(),
				query
				   );
	}
	 ...
	 * 		A custom database wrapper object to use.
	 * @param query
	 * 		The query to call.
	 *
	 * @return The data loaded from the database.
	...
	public TReturn load(@NonNull DatabaseWrapper databaseWrapper, String query, @Nullable TReturn data) {
		...
		final Cursor cursor = databaseWrapper.rawQuery(
				query,
				null
													  );
		return load(
	 ...
	 *
	 * @param cursor
	 * 		The cursor resulting from a query passed into {@link #load(String)}
	 * @param data
	 * 		The data (if not null) that we can reuse without need to create new object.
	 *
