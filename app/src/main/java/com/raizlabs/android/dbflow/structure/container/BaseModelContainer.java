...
public abstract class BaseModelContainer<TModel extends Model, DataClass>
	 ...
	 * 		The existing container to load into this one.
	 */
	public BaseModelContainer(@NonNull ModelContainer<TModel, ?> existingContainer) {
		...
		if (keys != null) {
			while (keys.hasNext()) {
				...
				put(
						key,
						existingContainer.getValue(key)
				   );
			}
		}
	}
	...
	/**
	 * @param inValue
	 * 		The value of data for a specified field.
	 * @param columnClass
	 * 		The class of the specified field/column
	 *
	...
	protected Object getModelValue(Object inValue, String columnName) {
		...
		if (columnAdapter != null) {
			inValue = columnAdapter.toModel(getInstance(
					inValue,
					columnClass
													   ));
		} else {
	 ...
	 *
	 * @param data
	 * 		The data object that backs this container
	 */
	@Override
	...
	public void putDefault(String columnName) {
		...
		Class columnClass = modelContainerAdapter.getColumnMap()
												 .get(columnName);
