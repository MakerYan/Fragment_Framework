...
public class Where<TModel extends Model>
	 ...
	 *
	 * @param whereBase
	 * 		The FROM or SET statement chunk
	 */
	 ...
	 *
	 * @param conditions
	 * 		The array of {@link SQLCondition}
	 *
	 * @return
	 */
	 ...
	 *
	 * @param where
	 * 		The query to use in the EXISTS clause. Such as SELECT * FROM `MyTable` WHERE ... etc.
	 *
	 * @return This where with an EXISTS clause.
	 */
	public Where<TModel> exists(@NonNull Where where) {

		conditionGroup.and(new ExistenceCondition().where(where));
		return this;
	}
	...
	public long count(DatabaseWrapper databaseWrapper) {
		...
		if ((whereBase instanceof Set) || whereBase.getQueryBuilderBase() instanceof Delete) {
			count = databaseWrapper.compileStatement(getQuery())
								   .executeUpdateDelete();
		} else {
			try {
				count = SqlUtils.longForQuery(
						databaseWrapper,
						getQuery()
											 );
	...
	public Cursor query() {
		...
		return query(FlowManager.getDatabaseForTable(getTable())
								.getWritableDatabase());
	}
