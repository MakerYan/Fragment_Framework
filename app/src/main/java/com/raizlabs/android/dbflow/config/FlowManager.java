...
public class FlowManager {
	...
	private static final String DEFAULT_DATABASE_HOLDER_PACKAGE_NAME = FlowManager.class.getPackage()
																						.getName();
	 ...
	 *
	 * @param table
	 * 		The class that implements {@link Model}
	 *
	...
	/**
	 * @param databaseName
	 * 		The name of the database. Will throw an exception if the database doesn't exist.
	 * @param tableName
	...
	public static Class<? extends Model> getTableClassForName(String databaseName, String tableName) {
		...
		if (databaseDefinition == null) {
			throw new IllegalArgumentException(String.format(
					"The specified database %1s was not found. " + "Did you forget to add the @Database?",
					databaseName
															));
		}
		...
		if (modelClass == null) {
			throw new IllegalArgumentException(String.format(
					"The specified table %1s was not found. " + "Did you forget to add the @Table annotation and point it to %1s?",
					tableName,
					databaseName
															));
		}
	 ...
	 *
	 * @param flowConfig
	 * 		The configuration instance that will help shape how DBFlow gets constructed.
	 */
	public static void init(@NonNull FlowConfig flowConfig) {
		...
		try {
			...
			FlowLog.log(
					FlowLog.Level.W,
					"Could not find the default GeneratedDatabaseHolder"
					   );
		}

		if (flowConfig.databaseHolders() != null && !flowConfig.databaseHolders()
															   .isEmpty()) {
	...
	/**
	 * @param modelClass
	 * 		The class that implements {@link Model} to find an adapter for.
	 *
	...
	public static InstanceAdapter getInstanceAdapter(Class<? extends Model> modelClass) {
		...
		if (internalAdapter == null) {
			if (BaseModelView.class.isAssignableFrom(modelClass)) {
				internalAdapter = FlowManager.getModelViewAdapter((Class<? extends BaseModelView<? extends Model>>) modelClass);
			} else if (BaseQueryModel.class.isAssignableFrom(modelClass)) {
				internalAdapter = FlowManager.getQueryModelAdapter((Class<? extends BaseQueryModel>) modelClass);
			}
		}
	...
	public static <TModel extends Model> ModelAdapter<TModel> getModelAdapter(Class<TModel> modelClass) {
		...
		return FlowManager.getDatabaseForTable(modelClass)
						  .getModelAdapterForTable(modelClass);
	}
	...
	public static <TModel extends Model> ModelContainerAdapter<TModel> getContainerAdapter(Class<TModel> modelClass) {
		...
		return FlowManager.getDatabaseForTable(modelClass)
						  .getModelContainerAdapterForTable(modelClass);
	}
	 ...
	 * @param <TModelView>
	 * 		The class that extends {@link BaseModelView}
	 *
	...
	public static <TModelView extends BaseModelView<? extends Model>> ModelViewAdapter<? extends Model, TModelView> getModelViewAdapter(Class<TModelView> modelViewClass) {
		...
		return FlowManager.getDatabaseForTable(modelViewClass)
						  .getModelViewAdapterForTable(modelViewClass);
	}
	 ...
	 * @param <TQueryModel>
	 * 		The class that extends {@link BaseQueryModel}
	 *
	...
	public static <TQueryModel extends BaseQueryModel> QueryModelAdapter<TQueryModel> getQueryModelAdapter(Class<TQueryModel> queryModel) {
		...
		return FlowManager.getDatabaseForTable(queryModel)
						  .getQueryModelAdapterForQueryClass(queryModel);
	}
	 ...
	 *
	 * @param databaseName
	 * 		The name of the database to check. Will thrown an exception if it does not exist.
	 *
	 * @return true if it's integrity is OK.
	 */
	public static boolean isDatabaseIntegrityOk(String databaseName) {

		return getDatabase(databaseName).getHelper()
