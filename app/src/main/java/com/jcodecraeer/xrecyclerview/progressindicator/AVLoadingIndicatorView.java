...
public class AVLoadingIndicatorView
	...
	private void init(AttributeSet attrs, int defStyle) {
		...
		mIndicatorId = a.getInt(
				R.styleable.AVLoadingIndicatorView_indicator,
				BallPulse
							   );
		mIndicatorColor = a.getColor(
				R.styleable.AVLoadingIndicatorView_indicator_color,
				Color.WHITE
									);
		a.recycle();
		mPaint = new Paint();
	...
	private int dp2px(int dpValue) {
		...
		return (int) getContext().getResources()
								 .getDisplayMetrics().density * dpValue;
	}
