...
public class JazzyViewPager
	...
	public JazzyViewPager(Context context, AttributeSet attrs) {
		...
		int effect = ta.getInt(
				R.styleable.JazzyViewPager_style,
				0
							  );
		...
		setFadeEnabled(ta.getBoolean(
				R.styleable.JazzyViewPager_fadeEnabled,
				false
									));
		...
		setOutlineColor(ta.getColor(
				R.styleable.JazzyViewPager_outlineColor,
				Color.WHITE
								   ));
		switch (mEffect) {
			case Stack:
	...
	private void logState(View v, String title) {
		...
		Log.v(
				TAG,
				title + ": ROT (" + ViewHelper.getRotation(v) + ", " + ViewHelper.getRotationX(v) + ", " + ViewHelper.getRotationY(v) + "), TRANS (" + ViewHelper.getTranslationX(v) + ", " + ViewHelper.getTranslationY(v) + "), SCALE (" + ViewHelper.getScaleX(v) + ", " + ViewHelper.getScaleY(v) + "), ALPHA " + ViewHelper.getAlpha(v)
			 );
	}
	...
	private void animateRotate(View left, View right, float positionOffset, boolean up) {
		...
		if (mState != State.IDLE) {
			if (left != null) {
				...
				mRot = (up ?
						1 :
						-1) * (ROT_MAX * positionOffset);
				mTrans = (up ?
						-1 :
						1) * (float) (getMeasuredHeight() - getMeasuredHeight() * Math.cos(mRot * Math.PI / 180.0f));
				ViewHelper.setPivotX(
						left,
						left.getMeasuredWidth() * 0.5f
									);
			...
			if (right != null) {
				...
				mRot = (up ?
						1 :
						-1) * (-ROT_MAX + ROT_MAX * positionOffset);
				mTrans = (up ?
						-1 :
						1) * (float) (getMeasuredHeight() - getMeasuredHeight() * Math.cos(mRot * Math.PI / 180.0f));
				ViewHelper.setPivotX(
	...
	protected float getOffsetXForRotation(float degrees, int width, int height) {
		...
		return (width - mTempFloat2[0]) * (degrees > 0.0f ?
				1.0f :
				-1.0f);
	}
	...
	public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
		...
		float effectOffset = isSmall(positionOffset) ?
				0 :
				positionOffset;
