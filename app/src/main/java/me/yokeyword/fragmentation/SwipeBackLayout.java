...
public class SwipeBackLayout
	...
	public void computeScroll() {
		...
		if (mScrimOpacity >= 0) {
			...
			if (mPreFragment != null && mPreFragment.getView() != null && mHelper.getCapturedView() != null) {
				int leftOffset = (int) ((mHelper.getCapturedView()
												.getLeft() - getWidth()) * mParallaxOffset * mScrimOpacity);
				mPreFragment.getView()
							.setLeft(leftOffset > 0 ?
											 0 :
											 leftOffset);
			}
		}
	...
	class ViewDragCallback
		...
		public boolean tryCaptureView(View child, int pointerId) {
			...
			boolean dragEnable = mHelper.isEdgeTouched(
					mEdgeFlag,
					pointerId
													  );
			if (dragEnable) {
				if (mHelper.isEdgeTouched(
						EDGE_LEFT,
						pointerId
										 )) {
					mCurrentSwipeOrientation = EDGE_LEFT;
				} else if (mHelper.isEdgeTouched(
						EDGE_RIGHT,
						pointerId
												)) {
				...
				if (mPreFragment == null) {
					if (mFragment != null) {
						List<Fragment> fragmentList = mFragment.getFragmentManager()
															   .getFragments();
		...
		public void onViewReleased(View releasedChild, float xvel, float yvel) {
			...
			int left = 0, top = 0;
			if ((mCurrentSwipeOrientation & EDGE_LEFT) != 0) {
				left = xvel > 0 || xvel == 0 && mScrollPercent > mScrollFinishThreshold ?
						(childWidth + mShadowLeft.getIntrinsicWidth() + OVERSCROLL_DISTANCE) :
						0;
			} else if ((mCurrentSwipeOrientation & EDGE_RIGHT) != 0) {
				left = xvel < 0 || xvel == 0 && mScrollPercent > mScrollFinishThreshold ?
						-(childWidth + mShadowRight.getIntrinsicWidth() + OVERSCROLL_DISTANCE) :
						0;
			}
